...
import coneysEnvironment from '../util/coneysEnvironment';
import coneysConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!coneysEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    coneysConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!coneysConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !coneysConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (coneysConfig.manageDaemonLifetime()) {
        coneysEnvironment.startConeysDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Coneys Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Coneys-Network/coneys-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Coneys-Network/coneys-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Coneys-Network/coneys-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Coneys-Network/coneys-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Coneys-Network/coneys-blockchain/issues',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal('https://twitter.com/coneys_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Coneys Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Coneys' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Coneys Blockchain' }),
            click: () => {
              openAbout();
            },
          },
