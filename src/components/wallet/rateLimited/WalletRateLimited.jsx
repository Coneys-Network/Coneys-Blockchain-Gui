...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const coneysper_input = ip_parsed.limit;
    ...
    const coneysper_value = Number.parseInt(Number(coneysper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (coneys per interval):{' '}
                  {mojo_to_coneys_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = coneys_to_mojo(fee_input.value);
